                    ...
                    switch (Convert.GetTypeCode(value, ic))
                    {
                        ...
                        case TypeCode.String:
                            ...
                            switch (target)
                            {
                                ...
                                case TypeCode.Single: try { return Single.Parse(str, CultureInfo.InvariantCulture); }
                                    catch { goto case TypeCode.Double; }
                                case TypeCode.Int64: try { return Int64.Parse(str, CultureInfo.InvariantCulture); }
                                    catch { goto case TypeCode.Double; }
                                case TypeCode.UInt64: try { return UInt64.Parse(str, CultureInfo.InvariantCulture); }
                                    catch { goto case TypeCode.Double; }
                                case TypeCode.Decimal: try { return Decimal.Parse(str, CultureInfo.InvariantCulture); }
                                    catch { goto case TypeCode.Double; }
                                case TypeCode.DateTime:
                                    try
                                    {
   ...
   /*String*/   { false, false, false, false,  false, false, false, false, false, false, false, false, false, false, false, false,  false,   false,   true}
      ...
      [DebuggerHiddenAttribute]
        ...
        internal static double ToNumber(String str, bool hexOK, bool octalOK, Object radix)
        {
            ...
            if (!octalOK)
            { //only parseInt will set octalOK. Integers can always be converted without COM+ help
                try
                {
                ...
                catch
                {
                    ...
                    { //The parseFloat case: all trailers are ignored. Prefix starting at i must match StrDecimalLiteral
                        //Might be "Infinity"
