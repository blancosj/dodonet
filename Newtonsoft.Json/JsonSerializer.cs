...
namespace Newtonsoft.Json
{
	...
	public class JsonSerializer
	{
		...
		public object Deserialize(JsonReader reader, Type objectType, params object[] args)
		{
			if (reader == null)
			...
			return GetObject(reader, objectType, args);
		}
			...
			while (reader.Read())
			{
				switch (reader.TokenType)
				{
					case JsonToken.PropertyName:
						...
						{
							if (!reader.Read())
								throw new Exception("Unexpected end while deserializing object.");
		...
		private object GetObject(JsonReader reader, Type objectType, params object[] args)
		{
			_level++;
			...
			else
			{
				switch (reader.TokenType)
				{
					...
					case JsonToken.StartObject:
						value = (objectType != null) ? PopulateObject(reader, objectType, args) : PopulateJavaScriptObject(reader);
						break;
		...
		private object EnsureType(object value, Type targetType)
		{
			...
			{
                if (valueType == typeof(string) && targetType == typeof(DateTime))
                {
                    // arreglamos formato 
                    var rex = Regex.Split((string)value, 
                        @"(?<Date>\d{4}-\d{2}-\d{2})\w{1}(?<Time>\d{2}:\d{2}:\d{2})", 
                    ...
                    foreach (Group item in rex.Groups)
                    {
                        tmpValue = string.Concat(tmpValue, item.Value);
                    }
                    */
                    value = String.Join(" ", rex, 1, 2);
                }
		...
		private void SetObjectMember(JsonReader reader, object target, Type targetType, string memberName)
		{
			...
			if (!CollectionUtils.IsNullOrEmpty<MemberInfo>(memberCollection))
			{
				...
				MemberInfo member = targetType.GetMember(memberName,
                    BindingFlags.NonPublic | BindingFlags.Instance |
                    BindingFlags.Public | BindingFlags.SetField |
			...
			{
				// attempt to the IDictionary's type
				memberType = ReflectionUtils.GetDictionaryValueType(target);
		...
		{
			IList list;
			Type elementType = ReflectionUtils.GetListType(objectType);
		...
		private object PopulateObject(JsonReader reader, Type objectType, params object[] args)
		{
			object newObject = Activator.CreateInstance(objectType, args);
			...
			while (reader.Read())
			{
				switch (reader.TokenType)
				{
					case JsonToken.PropertyName:
                        string memberName = reader.Value.ToString();
                        if (memberName.CompareTo(JsonConfig.memberNameForTypeName) != 0)
		...
		private void SerializeValue(JsonWriter writer, object value)
		{
            ...
            {
                SerializeDataSet(writer, (DataSet)value);
            }
            else if (value is DataTable)
            {
                SerializeDataTable(writer, (DataTable)value);
            }
		...
		private void SerializeObject(JsonWriter writer, object value)
		{
            ...
            List<MemberInfo> members = ReflectionUtils.GetFieldsAndProperties(objectType,
            ...
            // nombre del tipo
            writer.WritePropertyName(JsonConfig.memberNameForTypeName);
            SerializeValue(writer, objectType.Name);
        ...
        private void SerializeDataSet(JsonWriter writer, DataSet value)
        {
            writer.WriteStartArray();

            foreach (DataTable table in value.Tables)
            {
                SerializeDataTable(writer, table);
            }

            writer.WriteEndArray();
        }

        private void SerializeDataTable(JsonWriter writer, DataTable value)
        {
            writer.WriteStartObject();

            writer.WritePropertyName("TableName");
            SerializeValue(writer, value.TableName);
        ...
        private void SerializeDataRow(JsonWriter writer, DataRow value)
        {
            writer.WriteStartObject();
            ...
            {   
                ...
                SerializeValue(writer, value[column.ColumnName]);
            }
            writer.WriteEndObject();
        }
